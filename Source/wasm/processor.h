...
public:
    ...
    void set_centerDelay(float value) {
        centerDelay = std::clamp(value, 1.0f, 5.0f);
    }
    ...
    float get_centerDelay() const { return centerDelay; }
...
private:
    ...
    float feedback = 0.5f; // Range: -0.95 - 0.95
    float centerDelay = 2.0f; // Range: 1 - 5
