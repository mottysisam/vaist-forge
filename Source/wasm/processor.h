...
public:
    ...
    void set_rate(float value) {
        rate = std::clamp(value, 0.1f, 10.0f);
    }
    ...
    void set_manual(float value) {
        manual = std::clamp(value, 1.0f, 20.0f);
    }
    ...
    void set_offset(float value) {
        offset = std::clamp(value, 0.0f, 180.0f);
    }
...
private:
    ...
    // Parameters
    float rate = 0.5f; // Range: 0.1 - 10
    float depth = 75.0f; // Range: 0 - 100
    float manual = 5.0f; // Range: 1 - 20
    float feedback = 40.0f; // Range: -95 - 95
    float waveform = 0.0f; // Range: 0 - 3
    float offset = 90.0f; // Range: 0 - 180
