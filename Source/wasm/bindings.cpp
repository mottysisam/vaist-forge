 ...
 *     -s WASM=1 \
 *     -s EXPORTED_FUNCTIONS='["_prepare","_reset","_destroy","_process","_set_rate","_get_rate","_set_depth","_get_depth","_set_feedback","_get_feedback","_set_centerDelay","_get_centerDelay","_set_mix","_get_mix"]' \
...
EMSCRIPTEN_KEEPALIVE
void set_centerDelay(float value) {
    g_processor.set_centerDelay(value);
}
...
EMSCRIPTEN_KEEPALIVE
float get_centerDelay() {
    return g_processor.get_centerDelay();
}
